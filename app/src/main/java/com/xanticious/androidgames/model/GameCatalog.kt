package com.xanticious.androidgames.model

object GameCatalog {
    val allGames: List<GameDefinition> = listOf(
        game("age-of-empires-lite", "Age of Empires Lite", GameCategory.RTS),
        game("anagrams", "Anagrams", GameCategory.WORD),
        game("asteroids", "Asteroids", GameCategory.ACTION),
        game("backgammon", "Backgammon", GameCategory.BOARD),
        game("battleships", "Battleships", GameCategory.STRATEGY),
        game("boggle", "Boggle", GameCategory.WORD),
        game("breakout", "Breakout", GameCategory.ACTION),
        game("brick-breaker-plus", "Brick Breaker Plus", GameCategory.ACTION),
        game("bridge", "Bridge", GameCategory.CARD),
        game("bubbles-pop", "Bubbles Pop", GameCategory.ACTION),
        game("carcassonne-lite", "Carcassonne Lite", GameCategory.BOARD),
        game("checkers", "Checkers", GameCategory.STRATEGY),
        game("chess", "Chess", GameCategory.STRATEGY),
        game("connect-four", "Connect Four", GameCategory.STRATEGY),
        game("creative-sandbox", "Creative Sandbox", GameCategory.EXPLORATION_CREATIVE),
        game("cribbage", "Cribbage", GameCategory.CARD),
        game("crossword-mini", "Crossword Mini", GameCategory.WORD),
        game("dominoes", "Dominoes", GameCategory.BOARD),
        game("dominosa", "Dominosa", GameCategory.PUZZLE),
        game("draughts-variants", "Draughts Variants", GameCategory.STRATEGY),
        game("drift-racer", "Drift Racer", GameCategory.ACTION),
        game("eight-puzzle", "Eight Puzzle", GameCategory.PUZZLE),
        game("endless-runner", "Endless Runner", GameCategory.ACTION),
        game("falling-blocks", "Falling Blocks", GameCategory.PUZZLE),
        game("find-pairs", "Find Pairs", GameCategory.SEARCH),
        game("flood", "Flood", GameCategory.PUZZLE),
        game("freecell", "FreeCell", GameCategory.CARD),
        game("gomoku", "Gomoku", GameCategory.STRATEGY),
        game("guitar-hero", "Guitar Hero", GameCategory.RHYTHM),
        game("hangman", "Hangman", GameCategory.WORD),
        game("hearts", "Hearts", GameCategory.CARD),
        game("hex", "Hex", GameCategory.STRATEGY),
        game("hidden-objects", "Hidden Objects", GameCategory.SEARCH),
        game("hole-swallowing-game", "Hole Swallowing Game", GameCategory.ACTION),
        game("idle-city-builder", "Idle City Builder", GameCategory.IDLE),
        game("idle-factory", "Idle Factory", GameCategory.IDLE),
        game("idle-farm", "Idle Farm", GameCategory.IDLE),
        game("jigsaw", "Jigsaw", GameCategory.PUZZLE),
        game("kenken", "KenKen", GameCategory.PUZZLE),
        game("klondike-solitaire", "Klondike Solitaire", GameCategory.CARD),
        game("letter-drop", "Letter Drop", GameCategory.WORD),
        game("light-up", "Light Up", GameCategory.PUZZLE),
        game("logic-grid", "Logic Grid", GameCategory.PUZZLE),
        game("love-letter", "Love Letter", GameCategory.BOARD),
        game("mahjong-solitaire", "Mahjong Solitaire", GameCategory.PUZZLE),
        game("mastermind", "Mastermind", GameCategory.PUZZLE),
        game("match-three", "Match Three", GameCategory.PUZZLE),
        game("math-quiz", "Math Quiz", GameCategory.EDUCATIONAL),
        game("mazes", "Mazes", GameCategory.SEARCH),
        game("memory", "Memory", GameCategory.MEMORY),
        game("memory-lanes", "Memory Lanes", GameCategory.MEMORY),
        game("minesweeper", "Mine Sweeper", GameCategory.PUZZLE),
        game("nonogram", "Nonogram", GameCategory.PUZZLE),
        game("numberlink", "Numberlink", GameCategory.PUZZLE),
        game("othello", "Othello", GameCategory.STRATEGY),
        game("overcooked-lite", "Overcooked Lite", GameCategory.QUEUE_STRATEGY_ACTION),
        game("peg-solitaire", "Peg Solitaire", GameCategory.PUZZLE),
        game("pentomino", "Pentomino", GameCategory.PUZZLE),
        game("picross", "Picross", GameCategory.PUZZLE),
        game("pipes", "Pipes", GameCategory.PUZZLE),
        game("planet-explorer", "Planet Explorer", GameCategory.EXPLORATION_CREATIVE),
        game("poker", "Poker", GameCategory.CARD),
        game("pong", "Pong", GameCategory.ACTION),
        game("qwirkle-lite", "Qwirkle Lite", GameCategory.BOARD),
        game("randomized-dice-td", "Randomized Dice TD", GameCategory.TOWER_DEFENSE),
        game("reversi", "Reversi", GameCategory.STRATEGY),
        game("rhyme-rush", "Rhyme Rush", GameCategory.WORD),
        game("river-raid", "River Raid", GameCategory.ACTION),
        game("santa-tracker-maze", "Santa Tracker Maze", GameCategory.SEARCH),
        game("scrabble", "Scrabble", GameCategory.WORD),
        game("set-collector", "Set Collector", GameCategory.BOARD),
        game("shogi-lite", "Shogi Lite", GameCategory.STRATEGY),
        game("sim-city-blocks", "Sim City Blocks", GameCategory.EXPLORATION_CREATIVE),
        game("skyscrapers", "Skyscrapers", GameCategory.PUZZLE),
        game("sliding-puzzle", "Sliding Puzzle", GameCategory.PUZZLE),
        game("snake-io-solo", "Snake IO Solo", GameCategory.ACTION),
        game("snakes", "Snakes", GameCategory.ACTION),
        game("sokoban", "Sokoban", GameCategory.PUZZLE),
        game("solitaire", "Solitaire", GameCategory.CARD),
        game("space-invaders", "Space Invaders", GameCategory.ACTION),
        game("spades", "Spades", GameCategory.CARD),
        game("splendor", "Splendor", GameCategory.BOARD),
        game("star-miner", "Star Miner", GameCategory.EXPLORATION_CREATIVE),
        game("strategic-td", "Strategic TD", GameCategory.TOWER_DEFENSE),
        game("sudoku-colors", "Sudoku Colors", GameCategory.PUZZLE),
        game("swap-lines", "Swap Lines", GameCategory.PUZZLE),
        game("tafl-lite", "Tafl Lite", GameCategory.STRATEGY),
        game("tap-rhythm", "Tap Rhythm", GameCategory.RHYTHM),
        game("tetromino-sprint", "Tetromino Sprint", GameCategory.PUZZLE),
        game("tic-tac-toe", "Tic Tac Toe", GameCategory.STRATEGY),
        game("tower-stack", "Tower Stack", GameCategory.ACTION),
        game("trans-america", "Trans-America", GameCategory.BOARD),
        game("treasure-mapper", "Treasure Mapper", GameCategory.SEARCH),
        game("trivia-blitz", "Trivia Blitz", GameCategory.EDUCATIONAL),
        game("twenty-forty-eight", "2048", GameCategory.PUZZLE),
        game("word-chain", "Word Chain", GameCategory.WORD),
        game("word-ladder", "Word Ladder", GameCategory.WORD),
        game("word-search", "Word Search", GameCategory.SEARCH),
        game("wordle", "Wordle", GameCategory.WORD),
        game("wheres-waldo", "Where's Waldo", GameCategory.SEARCH),
        game("yahtzee", "Yahtzee", GameCategory.DICE),
        game("yatzy", "Yatzy", GameCategory.DICE),
        game("zilch", "Zilch", GameCategory.DICE),
        game("flash-cards-states", "Flash Cards - States", GameCategory.EDUCATIONAL),
        game("flash-cards-capitals", "Flash Cards - Capitals", GameCategory.EDUCATIONAL),
        game("flash-cards-countries", "Flash Cards - Countries", GameCategory.EDUCATIONAL),
        game("flash-cards-country-capitals", "Flash Cards - Country Capitals", GameCategory.EDUCATIONAL),
        game("anomaly-defense", "Anomaly Defense", GameCategory.TOWER_DEFENSE),
        game("base-defense", "Base Defense", GameCategory.TOWER_DEFENSE),
        game("resource-frontier", "Resource Frontier", GameCategory.RTS),
        game("empire-skirmish", "Empire Skirmish", GameCategory.RTS),
        game("city-merge-idle", "City Merge Idle", GameCategory.IDLE),
        game("idle-lab", "Idle Lab", GameCategory.IDLE),
        game("zen-garden", "Zen Garden", GameCategory.EXPLORATION_CREATIVE),
        game("color-quest", "Color Quest", GameCategory.PUZZLE),
        game("pathfinder", "Pathfinder", GameCategory.PUZZLE),
        game("chain-reaction", "Chain Reaction", GameCategory.STRATEGY),
        game("bomber-grid", "Bomber Grid", GameCategory.ACTION),
        game("rogue-caverns", "Rogue Caverns", GameCategory.EXPLORATION_CREATIVE),
        game("deck-builder-lite", "Deck Builder Lite", GameCategory.CARD),
        game("sevens", "Sevens", GameCategory.CARD),
        game("ludo", "Ludo", GameCategory.BOARD),
        game("reaction-speed", "Reaction Speed", GameCategory.RHYTHM),
        game("typing-sprint", "Typing Sprint", GameCategory.EDUCATIONAL)
    )

    private fun game(id: String, name: String, category: GameCategory): GameDefinition =
        GameDefinition(id = id, name = name, category = category)
}
