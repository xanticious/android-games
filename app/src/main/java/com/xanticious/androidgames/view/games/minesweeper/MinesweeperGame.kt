...
import com.xanticious.androidgames.view.common.puzzle.PuzzleCellScope
...
fun MinesweeperGame(difficulty: GameDifficulty, onExit: () -> Unit) {
    ...
    // Timer starts when mines are placed (after first tap) and pauses when not playing.
    ...
    PuzzleBoard(
        ...
        when {
            ...
            isFailed && cell.hasMine -> {
                ...
                drawRect(
                    color = if (isHit) PuzzleHighlight else PuzzleCell,
            ...
            // Wrongly flagged cell on failure (flag with no mine).
            ...
            cell.mark == CellMark.FLAGGED -> {
                ...
                drawFlagGlyph(GameHazard)
            }

            // Revealed cell with a neighbor count.
            cell.neighborCount > 0 -> {
                drawRect(color = PuzzleCell, topLeft = topLeft, size = cellSquare)
                val style = TextStyle(
                    color = neighborColor(cell.neighborCount),
                    fontSize = (cellSize * 0.45f / density).sp,
                    fontWeight = FontWeight.Bold
                )
                val layout = measurer.measure(AnnotatedString(cell.neighborCount.toString()), style)
                drawText(
                    layout, topLeft = Offset(
            ...
            // Revealed blank cell (flood-fill anchor).
            else -> drawRect(color = PuzzleCell, topLeft = topLeft, size = cellSquare)
        }
    }
}

/** Draws a flag glyph: vertical pole with a triangular pennant. */
private fun PuzzleCellScope.drawFlagGlyph(color: Color) {
...
/** Maps a neighbor mine count (1–8) to a distinct palette color. */
private fun neighborColor(count: Int): Color = when (count) {
    1 -> PuzzleHueBlue
    2 -> PuzzleHueGreen
    3 -> PuzzleHueRed
    4 -> PuzzleHueViolet
